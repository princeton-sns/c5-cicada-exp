...
data <- data %>%
    ...
    ) %>% filter(server %in% c("Primary", "KuaFu"))

# data

n_clients <- data %>%
    filter(server == "Primary") %>%
    group_by(n_warehouses, n_districts, n_customers, n_clients) %>%
    summarize(
        med_throughput_tps = median(throughput_tps)
    ) %>%
    group_by(n_warehouses, n_districts, n_customers) %>%
    mutate(
        max_med_throughput_tps = max(med_throughput_tps)
    ) %>%
    filter(med_throughput_tps == max_med_throughput_tps) %>%
    select(n_warehouses, n_districts, n_customers, n_clients)
...
n_workers <- data %>%
    ...
    group_by(server, n_warehouses, n_districts, n_customers, n_workers) %>%
    summarize(
        med_throughput_tps = median(throughput_tps)
    ) %>%
    group_by(server, n_warehouses, n_districts, n_customers) %>%
    mutate(
        max_med_throughput_tps = max(med_throughput_tps)
    ) %>%
    filter(med_throughput_tps == max_med_throughput_tps) %>%
    select(server, n_warehouses, n_districts, n_customers, n_workers)

# n_workers

n_workers_restricted <- n_workers %>%
    left_join(n_clients, by = c("n_warehouses", "n_districts", "n_customers")) %>%
    mutate(
...
data <- bind_rows(primary, backup) %>%
    mutate(
        n_customers = factor(n_customers),
    ) %>%
    group_by(server, n_warehouses, n_districts, n_customers) %>%
    summarize(
